...
import { getCorsHeaders, handleCorsPreflightOrRestrict, getBaseUrl } from '../_shared/cors.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${getBaseUrl(req)}/payments?success=true`,
      cancel_url: `${getBaseUrl(req)}/payments?canceled=true`,
      metadata: {
